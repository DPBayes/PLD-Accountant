...
def get_epsilon_PQ(P,Q,delta_target,ncomp=1E3,nx=3E6,L=30.0):
    ...
    delta_0 = np.real(np.sum((1-np.exp(eps0-grid_x[j_start:]))*y[j_start:]))
...
P,Q = get_PQ2(omega,omega2,nx=nx_,L=L_)
...
for k in ks:
    ...
    eps0=get_epsilon_PQ(P,Q,delta_target,ncomp=k,nx=nx_,L=L_)
    epsilons.append(eps0)
    print('k: ' + str(k) + ' eps: ' + str(eps0))


pickle.dump(epsilons, open("./pickles/eps_fig1_pld.p", "wb"))

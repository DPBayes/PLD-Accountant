...
def get_deltas(n1, p, D2, n_comp, nx=int(4E6), L=1000, target_eps=1.0):
    ...
    n_iter_per_epochs = 1
    start=4
    ks=np.linspace(start, n_comp*n_iter_per_epochs, n_comp-start+1)
    ...
    for (ijk,k) in enumerate(ks):

        lambdas=np.linspace(1.1,200,200)
        epss=[]
        for ll in lambdas:
        ...
        lambdas=np.linspace(1.1,30,100)
        eds=[]
        for ll in lambdas:
        ...
        deltas[ijk]=dd
        if(k%4==0):
            print( ('%1d' % k) + '  &  ' + ('%10.2E' % ep) + ' & ' + ('%10.2E' % ed) + ' & ' + ('%10.5E' % dd) + '\\\\')
    return deltas

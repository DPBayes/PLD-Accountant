...
Tests for vector variant of delta computation
...
class compute_delta_regression_tests(unittest.TestCase):
    ...
    def test_get_delta_R_invalid_sizes(self):
        with self.assertRaises(ValueError):
            get_delta_R(sigma_t = np.ones(2), q_t = np.ones(2), k = np.ones(1))
            get_delta_R(sigma_t = np.ones(2), q_t = np.ones(1), k = np.ones(2))
            get_delta_R(sigma_t = np.ones(1), q_t = np.ones(2), k = np.ones(2))
    ...
    def test_get_delta_S_invalid_sizes(self):
        with self.assertRaises(ValueError):
            get_delta_S(sigma_t = np.ones(2), q_t = np.ones(2), k = np.ones(1))
            get_delta_S(sigma_t = np.ones(2), q_t = np.ones(1), k = np.ones(2))
            get_delta_S(sigma_t = np.ones(1), q_t = np.ones(2), k = np.ones(2))
